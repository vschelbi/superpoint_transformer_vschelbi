...
from superpoint_transformer.partition.utils import point_utils
import colorsys
...
def read_semantic3d_format(data_file, n_class, file_label_path, voxel_width, ver_batch):
    ...
    def process_chunk(vertex_chunk, label_chunk, has_labels, xyz, rgb, labels):
        ...
        if voxel_width > 0:
            if has_labels > 0:
                xyz_sub, rgb_sub, labels_sub, objets_sub = point_utils.prune(xyz_full, voxel_width
                                             , rgb_full, labels_full , np.zeros(1, dtype='uint8'), n_class, 0)
                labels = np.vstack((labels, labels_sub))
                del labels_full
            else:
                xyz_sub, rgb_sub, l, o = point_utils.prune(xyz_full, voxel_width
